...
class MedicalResponse(BaseModel):
    ...
    priority_level: Optional[str] = "moderate"
            ...
            "follow_up_suggestions": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "priority": {"type": "string", "enum": ["critical", "high", "moderate", "low"], "description": "Priority level for medical urgency"},
                        "tags": {"type": "array", "items": {"type": "string"}, "description": "Tags like 'lifesaving', 'surgery-prep', 'emergency', etc."}
                    }
                },
                "description": "5 intelligent follow-up conversation starters with priority levels"
            },
            "priority_level": {
                "type": "string",
                "enum": ["critical", "high", "moderate", "low"],
                "description": "Overall priority level of this medical information"
            },
            "priority_tags": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Priority tags like 'lifesaving-drugs', 'surgery-prep', 'emergency-care', 'time-sensitive'"
            }
        },
...
class ConversationOptimizer:
    ...
    async def generate_medical_response(self, query: str, patient_context: Dict = None, conversation_history: List = None) -> Dict:
        ...
        try:
                ...
                messages=[
                    {
                        ...
                        - Assign priority levels: 'critical' for life-threatening/emergency situations, 'high' for urgent care needs, 'moderate' for important health matters, 'low' for general wellness
                        - Tag responses with relevant priority markers like 'lifesaving-drugs', 'surgery-prep', 'emergency-care', 'time-sensitive'
                        - Prioritize follow-ups that could be lifesaving or prevent serious complications
            ...
            cleaned_response = clean_json_string(function_response)
    ...
    async def modify_response(self, response_id: str, modification_type: str, patient_context: Dict = None) -> Dict:
        ...
        try:
            ...
            # Extract content based on modification type with better error handling
            content_key_map = {
                "simplify": "simplified_content",
                "detail": "detailed_content", 
                "personalize": "personalized_content"
            }
            
            content_key = content_key_map.get(modification_type)
            if not content_key or content_key not in result:
                # Fallback to any available content key
                possible_keys = ["simplified_content", "detailed_content", "personalized_content", "medical_content", "content"]
                content_key = next((key for key in possible_keys if key in result), None)
                
                if not content_key:
                    raise ValueError(f"No valid content key found in result: {list(result.keys())}")
            
            return {
                "id": new_response_id,
                "content": result[content_key],
                "modification_type": modification_type,
                "follow_up_prompts": result.get("follow_up_suggestions", []),
